...
class ColorizationCINN(nn.Module):
    ...
    def __init__(self,optimizer=None):
        ...
        if config.cond_net=="resnet_50":
            self.cond_net=resnet50_backbone()
    ...
    def build_inn(self,cond_features):
        ...
        # 0 2*64*64
        subnet = sub_conv(config.sub_conv[0], 3)
        for k in range(4):
        ...
        # 1 8*32*32
        for k in range(6):
            subnet = sub_conv(config.sub_conv[1], 3 if k%2 else 1)
        ...
        nodes.append(Ff.Node(nodes[-1], Fm.Split,
                             {'section_sizes':[16,16], 'dim':0}))
        split_nodes.append(Ff.Node(nodes[-1].out1, Fm.Flatten, {}))

        # 2
        for k in range(6):
            subnet = sub_conv(config.sub_conv[2], 3 if k%2 else 1)
        ...
        #64*8*8
        nodes.append(Ff.Node(nodes[-1], Fm.Split,
                             {'section_sizes':[32,32], 'dim':0}))
        split_nodes.append(Ff.Node(nodes[-1].out1, Fm.Flatten, {}))

        # 3
        for k in range(6):
            subnet = sub_conv(config.sub_conv[3], 3 if k%2 else 1)
            nodes.append(Ff.Node(nodes[-1], Fm.GLOWCouplingBlock,
        ...
        #128*4*4
        nodes.append(Ff.Node(nodes[-1], Fm.Split,
                             {'section_sizes':[32,96], 'dim':0}))
        split_nodes.append(Ff.Node(nodes[-1].out1, Fm.Flatten, {})) 
        nodes.append(Ff.Node(nodes[-1], Fm.Flatten, {}, name='flatten'))

        # 4
        subnet = sub_fc(config.sub_conv[4])
        for k in range(8):
            nodes.append(Ff.Node(nodes[-1], Fm.GLOWCouplingBlock,
                                 {'subnet_constructor':subnet, 'clamp':0.6},
    ...
    def forward(self, Lab,feature):
        #lab取ab通道
        ab=Lab[:,1:]
        ab = F.interpolate(ab, size=(config.ab_h_w,config.ab_h_w))
        z,jac = self.cinn(ab, c=self.cond_net(feature),jac=True)
        return z, jac

    def reverse_sample(self, z, feature):
        return self.cinn(z, c=self.cond_net(feature), rev=True)
