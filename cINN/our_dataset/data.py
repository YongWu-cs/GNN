...
def norm_lab_to_rgb(L, ab, norm=True):
    if isinstance(L, torch.Tensor):
        L=np.array(L)
        ab = np.array(ab)
    L = L.transpose((0, 2, 3, 1)).astype(dtype=np.float32)
    ab = ab.transpose((0, 2, 3, 1)).astype(dtype=np.float32)
...
class ApplyTransformToMultipleImages:
...
class LabColorDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            img_padding = self.to_tensor(img_padding).numpy()
            im = np.transpose(im, (1,2,0))
            img_padding = np.transpose(img_padding, (1,2,0))
            if im.shape[2] != 3:
                im = np.stack([im[:,:,0]]*3, axis=2)
            if img_padding.shape[2] != 3:
                img_padding = np.stack([img_padding[:,:,0]]*3, axis=2)
            im = color.rgb2lab(im).transpose((2, 0, 1))
            img_padding=color.rgb2lab(img_padding).transpose((2, 0, 1))
            for i in range(3):
                im[i] = (im[i] - offsets[i]) / scales[i]
                img_padding[i] = (img_padding[i] - offsets[i]) / scales[i]
            im = torch.Tensor(im)
            img_padding = torch.Tensor(img_padding)
        except:
            im=self.transform([im])[0]
            im = self.to_tensor(im).numpy()
            im = np.transpose(im, (1,2,0))
            if im.shape[2] != 3:
                im = np.stack([im[:,:,0]]*3, axis=2)
            ...
            for i in range(3):
                im[i] = (im[i] - offsets[i]) / scales[i]
            im = torch.Tensor(im)
            img_padding=im[:1].repeat(3,1,1)

        if self.noise:
            im += 0.005 * torch.rand_like(im) 

        if self.mode=="test":
            return im,img_padding,self.files[idx]
        else:
            return im,img_padding
...
transf_train = ApplyTransformToMultipleImages(transf_train)

transf_val_test = T.Compose([
    T.Resize(size=(config.image_h_w,config.image_h_w)),
])
transf_val_test = ApplyTransformToMultipleImages(transf_val_test)
...
if config.mode=="train":
    ...
    train_data = LabColorDataset(train_list, transf_train, noise=True,mode="train")
    val_data  =  LabColorDataset(val_list,  transf_val_test,mode="val")
