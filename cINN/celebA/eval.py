...
def colorize_test_set(cinn,temp=1., num=8, img_folder='base_model_result'):
    for i in range(num):
        ...
        with torch.no_grad():
            for Lab,img_p in tqdm(data.test_loader):
                img_name=[]
                for p in img_p:
                ...
                ab_gen,jac = cinn.reverse_sample(z, L)
...
def record_compare_pic(wandb,n,img_folder,test_folder="autodl-tmp/dataset/test"):
    ...
    for idx in tqdm(range(len(random_base_name))):
        ...
        base_pic=base_pic.resize((64,64))
...
if __name__=="__main__":
    model_path="autodl-tmp/celebeA_model/check_point/cond_net_celebA_model_10.pt"
    img_folder='autodl-tmp/celebA_cond_net/result'
    test_folder='autodl-tmp/celebeA/test'
    num=8
    ...
    if use_log:
        ...
        #record_compare_pic(wandb,num,img_folder)
        wandb.finish()
